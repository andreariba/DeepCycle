...
def plot_training(fit):
    ...
    ax[0].legend()
    ax[1].plot(fit.epoch,fit.history['val_kullback_leibler_divergence'],'.-',color='blue', label='validation')
    ax[1].plot(fit.epoch,fit.history['kullback_leibler_divergence'],'.-',color='green', label='train')
    ax[1].set_yscale('log')
    ax[1].set(xlabel='epochs',ylabel='KL divergence')
